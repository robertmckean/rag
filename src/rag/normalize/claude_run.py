"""Claude-only normalized run writing."""
...
def write_claude_normalized_run(
    ...
    *,
    ...
    """Extract Claude records and write one normalized run folder."""
    canonical_conversations, canonical_messages = extract_claude_records(conversations_path)
    ...
    manifest = {
        ...
        "providers_present": ["claude"],
        "source_roots": [str(conversations_path.parent)],
        "conversation_count": len(canonical_conversations),
        "message_count": len(canonical_messages),
        "notes": [
            "Claude-only phase-1 normalization run.",
            "Excluded sidecar streams: memories.json, projects.json, users.json.",
            "Attachment and file references preserved without copying blobs.",
        ],
    }
